module Podflow
  class Episode < FormattedConfigFile
    ...
    def email
      PodUtils.with_error_handling { series.email.each {|email| email.send(series, self)} }
    end

    def slack
      PodUtils.with_error_handling { series.slack.each {|slack| slack.send(series, self)} }
    end
